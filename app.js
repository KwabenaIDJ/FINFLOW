...
🔗 Generated on ${new Date().toLocaleDateString()}`;
    ...
    // 23. OCR Receipt Scanner Handler with Google Gemini Vision AI (Premium Feature)
    // Lookup the scan receipt button element from the DOM
    const scanReceiptBtn = document.getElementById('scanReceiptBtn');
    // Lookup the hidden file input element for selecting or capturing receipt photos
    const receiptFileInput = document.getElementById('receiptFileInput');
    // Lookup the inline status container for receipt scanning feedback
    const receiptScanStatusBox = document.getElementById('receiptScanStatusBox');
    // Check if both scan button and file input elements exist on the page
    if (scanReceiptBtn && receiptFileInput) {
      // Helper function to compress and encode an image file into Base64 for Gemini Vision API
      const compressAndEncodeReceiptImage = (file) => {
        // Return a promise that resolves with base64 data and mime type
        return new Promise((resolve, reject) => {
          // Instantiate a FileReader to read image binary data as a Data URL
          const reader = new FileReader();
          // Event handler triggered when FileReader finishes reading file
          reader.onload = (readerEvent) => {
            // Instantiate an Image object to load and decode the image
            const img = new Image();
            // Event handler triggered once image is fully decoded and dimensions are ready
            img.onload = () => {
              // Set maximum width or height dimension to reduce payload size and latency
              const MAX_DIM = 1280;
              // Store initial image width
              let width = img.width;
              // Store initial image height
              let height = img.height;
              // Downscale width and height proportionally if width exceeds maximum dimension
              if (width > height && width > MAX_DIM) {
                // Calculate proportional height
                height = Math.round((height * MAX_DIM) / width);
                // Clamp width to maximum allowed dimension
                width = MAX_DIM;
              // Downscale height and width proportionally if height exceeds maximum dimension
              } else if (height > MAX_DIM) {
                // Calculate proportional width
                width = Math.round((width * MAX_DIM) / height);
                // Clamp height to maximum allowed dimension
                height = MAX_DIM;
              // End conditional scaling
              }
              // Create offscreen canvas for resizing
              const canvas = document.createElement('canvas');
              // Assign scaled width to canvas
              canvas.width = width;
              // Assign scaled height to canvas
              canvas.height = height;
              // Retrieve 2D drawing context from canvas
              const ctx = canvas.getContext('2d');
              // Render resized image into canvas context
              ctx.drawImage(img, 0, 0, width, height);
              // Export canvas content as JPEG base64 data URL with 0.85 quality compression
              const dataUrl = canvas.toDataURL('image/jpeg', 0.85);
              // Extract pure base64 data portion by removing MIME prefix
              const base64Data = dataUrl.split(',')[1];
              // Resolve promise with base64 string and jpeg MIME identifier
              resolve({ base64Data, mimeType: 'image/jpeg' });
            // End image onload listener
            };
            // Error handler if image fails to decode properly
            img.onerror = () => reject(new Error('Unable to parse or decode the selected image file.'));
            // Set image source to Data URL to initiate decoding
            img.src = readerEvent.target.result;
          // End reader onload listener
          };
          // Error handler if FileReader encounters a file access error
          reader.onerror = () => reject(new Error('Unable to read selected receipt file.'));
          // Read the selected file as a Data URL
          reader.readAsDataURL(file);
        // End Promise wrapper
        });
      // End compressAndEncodeReceiptImage helper definition
      };

      // Helper function to call Google Gemini 1.5 Flash Vision API for receipt analysis
      const analyzeReceiptWithGeminiVision = async (base64Data, mimeType, apiKey) => {
        // Prepare current date in ISO format for default receipt date resolution
        const todayIso = new Date().toISOString().split('T')[0];
        // Construct explicit AI prompt instructing Gemini to classify receipt vs non-receipt and extract details
        const ocrPrompt = `You are an expert financial receipt, invoice, and bill scanner.
Analyze this image carefully.

Determine if this image is a genuine financial document (store receipt, restaurant bill, supermarket slip, invoice, utility bill, ticket, payment receipt, tax invoice, bank transfer voucher, or proof of purchase).

If the image is NOT a receipt, invoice, bill, or financial document (for example: a selfie, portrait, random scenery, animal, pet, meme, vehicle photo without pricing, artwork, or an unrelated document):
You MUST return JSON with:
{
  "is_receipt": false,
  "reason": "Clear, friendly explanation of what the image shows and why it contains no transaction or receipt details."
}

If the image IS a financial document or receipt:
Extract all available details accurately and return JSON with:
{
  "is_receipt": true,
  "merchant": "Vendor, store, or service provider name (e.g. Starbucks, Shell, Walmart)",
  "amount": 12.34,
  "category": "Food" | "Shopping" | "Transport" | "Utilities" | "Rent" | "Entertainment" | "Travel" | "Other",
  "date": "YYYY-MM-DD",
  "type": "expense",
  "reason": "Summary of detected transaction items"
}

Rules:
1. amount must be the final total paid as a positive number (float or integer, no currency symbols).
2. category must be one of: "Food", "Shopping", "Transport", "Utilities", "Rent", "Entertainment", "Travel", "Other".
   - Groceries, meals, drinks, cafe -> "Food"
   - Ride-hailing, gas, fuel, taxi, transit -> "Transport"
   - Water, electricity, cell recharge, MoMo transfer fees, internet -> "Utilities"
   - Retail, electronics, apparel, clothing -> "Shopping"
   - Rent or tenancy -> "Rent"
   - Cinema, games, events -> "Entertainment"
3. date must be formatted strictly as YYYY-MM-DD. If no date is visible, use "${todayIso}".
4. type should be "expense" unless explicitly an income payment receipt.`;

        // 1. Try Vercel Serverless Function (/api/ai-coach) if available
        try {
          // Make HTTP POST request to serverless endpoint
          const apiRes = await fetch('/api/ai-coach', {
            // HTTP POST method
            method: 'POST',
            // Set application/json content type header
            headers: { 'Content-Type': 'application/json' },
            // Send prompt along with inline image data in JSON payload
            body: JSON.stringify({
              // Prompt instruction
              prompt: ocrPrompt,
              ...
              // Image MIME type
              mimeType: mimeType
            // End request body JSON serialization
            })
          // End fetch invocation
          });
          // Check if serverless request returned success HTTP status
          if (apiRes.ok) {
            // Parse serverless response JSON
            const serverData = await apiRes.json();
            // Check if serverless returned a text response
            if (serverData && serverData.text) {
              // Clean any markdown code fences surrounding JSON response
              const cleanedText = serverData.text.replace(/```json/gi, '').replace(/```/g, '').trim();
              // Return parsed JSON object
              return JSON.parse(cleanedText);
            // End check for serverData.text
            }
          // End check for apiRes.ok
          }
        // Catch serverless network errors and fall through to direct Gemini API call
        } catch (serverlessErr) {
          // Log serverless fallback warning in browser console
          console.warn('Serverless endpoint unavailable, falling back to direct Gemini API:', serverlessErr);
        // End try-catch for serverless proxy
        }

        // 2. Direct Gemini 1.5 Flash Vision REST API call
        // Construct official Gemini 1.5 Flash generateContent endpoint URL with API key
        const geminiUrl = `https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=${apiKey}`;
        // Construct direct Gemini API request body payload
        const requestPayload = {
          // Contents array containing prompt text and inline base64 image
          contents: [
            // Single content block
            {
              // Array of parts for multimodal inference
              parts: [
                // Text prompt part instructing OCR extraction
                { text: ocrPrompt },
                // Inline image data part containing encoded receipt image
                {
                  // Inline data container object
                  inline_data: {
                    // MIME type for encoded image
                    mime_type: mimeType,
                    // Base64 encoded image string
                    data: base64Data
                  // End inline_data object
                  }
                // End inline image part
                }
              // End parts array
              ]
            // End content block
          ...
          // Generation configuration requesting structured JSON response MIME type
          generationConfig: {
            // Set output MIME type to application/json for deterministic parsing
            response_mime_type: 'application/json'
          // End generationConfig object
          }
        // End requestPayload object
        };

        // Execute asynchronous HTTP POST request to Google Gemini API
        const response = await fetch(geminiUrl, {
          // Use HTTP POST method
          method: 'POST',
          // Specify application/json content header
          headers: { 'Content-Type': 'application/json' },
          // Serialize request payload to JSON string
        ...
        if (!response.ok) {
          ...
          // Throw descriptive error with HTTP status or API error details
          throw new Error(errData?.error?.message || `Gemini API responded with HTTP status ${response.status}`);
        // End HTTP validation check
        }

        // Parse JSON response from Gemini API
        const responseData = await response.json();
        // Extract generated text content from first candidate part
        const rawText = responseData?.candidates?.[0]?.content?.parts?.[0]?.text;
        // Check if generated text was returned
        if (!rawText) {
          // Throw error if Gemini did not return text content
          throw new Error('Gemini API returned an empty or invalid response.');
        // End text existence check
        }

        // Clean any residual code block markup from model output
        const cleanJsonStr = rawText.replace(/```json/gi, '').replace(/```/g, '').trim();
        // Parse and return structured JSON result
        return JSON.parse(cleanJsonStr);
      // End analyzeReceiptWithGeminiVision definition
      };

      // Click event listener for receipt scanner button
      scanReceiptBtn.addEventListener('click', () => {
        // Retrieve membership status from global application store
        const isPremium = window.AppStore ? window.AppStore.getSettings().isPremium : false;
        // Restrict OCR scanning feature to Premium subscribers
        if (!isPremium) {
          // Open Premium upgrade modal
          window.openPremiumModal();
          // Alert user about premium requirement for AI receipt scanner
          ...
          // Terminate click handler
          return;
        // End premium membership check
        }
        // Programmatically trigger native file input dialog
        receiptFileInput.click();
      // End scanReceiptBtn click listener
      });

      // Change event listener triggered when user selects or captures a photo
      receiptFileInput.addEventListener('change', async (e) => {
        // Retrieve selected image file from file list
        const file = e.target.files[0];
        // Abort processing if no file was selected
        if (!file) return;

        // Retrieve current application store settings
        const store = window.AppStore;
        // Extract settings object
        const settings = store ? store.getSettings() : {};
        // Retrieve Gemini API key from hardcoded constant or store settings
        let apiKey = HARDCODED_GEMINI_API_KEY || (settings && settings.geminiApiKey ? settings.geminiApiKey : '');

        // Check if Gemini API key is missing
        if (!apiKey) {
          // Prompt user to enter their Gemini API key to enable AI OCR scanning
          const enteredKey = prompt('🔑 Google Gemini API Key Required:\n\nTo enable Smart Receipt OCR Vision scanning, please enter your free Google Gemini API Key (or configure it in Settings > Gemini API Key):');
          // If user entered a key, persist it to application settings
          if (enteredKey && enteredKey.trim()) {
            // Trim whitespace from entered API key
            apiKey = enteredKey.trim();
            // Update settings object with entered key
            settings.geminiApiKey = apiKey;
            // Persist updated settings to local storage store
            if (store && store.saveSettings) store.saveSettings(settings);
          // Otherwise warn user and abort scanning
          } else {
            // Display alert instructing user to enter key
            alert('⚠️ Gemini API Key Required: Please provide your Google Gemini API Key in Settings to use Smart Receipt OCR scanning.');
            // Reset file input value to allow re-selection
            receiptFileInput.value = '';
            // Exit change handler
            return;
          // End API key prompt check
          }
        // End API key check
        }

        // Update button appearance to reflect active scanning state
        scanReceiptBtn.innerHTML = '⌛ Scanning Receipt with Gemini Vision...';
        // Disable button during active processing
        scanReceiptBtn.disabled = true;
        // Set active styling color
        scanReceiptBtn.style.color = '#a855f7';
        // Set active styling border color
        scanReceiptBtn.style.borderColor = 'rgba(168, 85, 247, 0.5)';

        // Display inline status container if element exists
        if (receiptScanStatusBox) {
          // Show status box
          receiptScanStatusBox.style.display = 'block';
          // Set loading status box background color
          receiptScanStatusBox.style.background = 'rgba(168, 85, 247, 0.12)';
          // Set loading status box text color
          receiptScanStatusBox.style.color = '#c084fc';
          // Set loading status box border styling
          receiptScanStatusBox.style.border = '1px solid rgba(168, 85, 247, 0.3)';
          // Display analyzing message in status box
          receiptScanStatusBox.innerHTML = '🔍 Reading photo & analyzing receipt with Gemini AI Vision...';
        // End status box check
        }

        // Begin asynchronous scanning try block
        try {
          // Compress and encode selected image into Base64 string
          const { base64Data, mimeType } = await compressAndEncodeReceiptImage(file);
          // Call Gemini Vision OCR API to extract transaction details
          const result = await analyzeReceiptWithGeminiVision(base64Data, mimeType, apiKey);

          // Check if uploaded photo is NOT a valid receipt or transaction document
          if (!result || result.is_receipt === false) {
            // Build explanatory message for non-receipt photo
            const reasonMsg = result?.reason || 'FinFlow could not find any transaction or pricing details in this image. Please upload a clear photo of a store receipt, utility bill, or payment invoice.';
            // Update button label to indicate photo was not a receipt
            scanReceiptBtn.innerHTML = '⚠️ Not a receipt photo';
            // Set error color on button
            scanReceiptBtn.style.color = '#ef4444';
            // Set error border on button
            scanReceiptBtn.style.borderColor = 'rgba(239, 68, 68, 0.4)';
            // Re-enable button
            scanReceiptBtn.disabled = false;

            // Update inline status box with friendly explanation
            if (receiptScanStatusBox) {
              // Ensure status box remains visible
              receiptScanStatusBox.style.display = 'block';
              // Set warning background on status box
              receiptScanStatusBox.style.background = 'rgba(239, 68, 68, 0.12)';
              // Set warning text color on status box
              receiptScanStatusBox.style.color = '#f87171';
              // Set warning border on status box
              receiptScanStatusBox.style.border = '1px solid rgba(239, 68, 68, 0.3)';
              // Render non-receipt warning message in status box
              receiptScanStatusBox.innerHTML = `<strong>⚠️ No Receipt Detected:</strong> ${reasonMsg}`;
            // End status box update
            }

            // Display modal alert informing user that no transaction details were detected
            alert(`⚠️ No Receipt Detected\n\n${reasonMsg}`);

            // Reset button to initial state after 3.5 seconds
            setTimeout(() => {
              // Restore initial button label
              scanReceiptBtn.innerHTML = '📷 Snap or Upload Receipt Photo';
              // Restore initial button color
              scanReceiptBtn.style.color = '#a855f7';
              // Restore initial button border
              scanReceiptBtn.style.borderColor = 'rgba(168, 85, 247, 0.3)';
            // End timeout delay
            }, 3500);

            // Terminate handler without modifying form input fields
            return;
          // End non-receipt photo branch
          }

          // Reference transaction amount input field
          const amountInput = document.getElementById('addTxAmount');
          // Reference transaction category select dropdown
          const catInput = document.getElementById('addTxCategory');
          // Reference transaction description input field
          const descInput = document.getElementById('addTxDesc');
          // Reference transaction date input field
          const dateInput = document.getElementById('addTxDate');
          // Reference transaction type selector
          const typeInput = document.getElementById('addTxType');

          // Check if parsed amount is a valid positive number
          if (amountInput && result.amount !== null && result.amount !== undefined && !isNaN(Number(result.amount))) {
            // Format and auto-fill amount field to two decimal places
            amountInput.value = Math.abs(Number(result.amount)).toFixed(2);
          // End amount auto-fill
          }

          // Check and update transaction type selector if specified
          if (typeInput) {
            // Set transaction type to income or default expense
            typeInput.value = result.type === 'income' ? 'income' : 'expense';
            // Synchronize category dropdown options with selected transaction type
            if (catInput) updateCategoryDropdown(typeInput, catInput);
          // End type selector update
          }

          // Map and auto-fill category field
          if (catInput) {
            // Default category target to Other
            let targetCategory = 'Other';
            // Extract returned category string in lower case
            const returnedCategory = (result.category || '').toLowerCase();
            // Extract returned merchant or description in lower case
            const descLower = (result.merchant || result.description || '').toLowerCase();
            // Check for Food & Dining related categories
            if (returnedCategory.includes('food') || returnedCategory.includes('grocer') || returnedCategory.includes('dine') || returnedCategory.includes('restaurant') || returnedCategory.includes('meal')) {
              // Set target category to Food
              targetCategory = 'Food';
            // Check for Transport / Travel related categories
            } else if (returnedCategory.includes('transport') || returnedCategory.includes('travel') || returnedCategory.includes('fuel') || returnedCategory.includes('uber') || returnedCategory.includes('gas') || descLower.includes('fuel') || descLower.includes('gas') || descLower.includes('uber') || descLower.includes('bolt')) {
              // Choose Transport or Travel depending on dropdown option availability
              targetCategory = Array.from(catInput.options).some(opt => opt.value === 'Transport') ? 'Transport' : 'Travel';
            // Check for Utilities related categories
            } else if (returnedCategory.includes('util') || returnedCategory.includes('bill') || returnedCategory.includes('electric') || returnedCategory.includes('water') || returnedCategory.includes('airtime') || returnedCategory.includes('momo')) {
              // Set target category to Utilities
              targetCategory = 'Utilities';
            // Check for Shopping related categories
            } else if (returnedCategory.includes('shop') || returnedCategory.includes('store') || returnedCategory.includes('retail') || returnedCategory.includes('mart') || returnedCategory.includes('cloth')) {
              // Set target category to Shopping
              targetCategory = 'Shopping';
            // Check for Rent related categories
            } else if (returnedCategory.includes('rent') || returnedCategory.includes('lease')) {
              // Set target category to Rent
              targetCategory = 'Rent';
            // Check for Entertainment related categories
            } else if (returnedCategory.includes('entertain') || returnedCategory.includes('movie') || returnedCategory.includes('game')) {
              // Set target category to Entertainment
              targetCategory = 'Entertainment';
            // Check if returned category directly matches any existing option in dropdown
            } else if (Array.from(catInput.options).some(opt => opt.value.toLowerCase() === returnedCategory)) {
              // Set target category to the matching option value
              targetCategory = Array.from(catInput.options).find(opt => opt.value.toLowerCase() === returnedCategory).value;
            // End category condition checks
            }
            // Assign selected category value to dropdown
            catInput.value = targetCategory;
          // End category assignment
          }

          // Auto-fill description with merchant name or summary
          if (descInput) {
            // Assign merchant name or fallback description
            descInput.value = result.merchant || result.description || 'Receipt Purchase';
          // End description auto-fill
          }

          // Auto-fill transaction date if valid ISO date string returned
          if (dateInput && result.date) {
            // Attempt date parsing
            const parsedDate = new Date(result.date);
            // Verify date validity
            if (!isNaN(parsedDate.getTime())) {
              // Assign formatted date string
              dateInput.value = result.date.substring(0, 10);
            // End date validation check
            }
          // End date auto-fill
          }

          // Update scan button to indicate success
          scanReceiptBtn.innerHTML = '✅ Receipt Scanned! Details Auto-Filled';
          // Set success color styling
          scanReceiptBtn.style.color = '#10b981';
          // Set success border styling
          scanReceiptBtn.style.borderColor = 'rgba(16, 185, 129, 0.4)';
          // Re-enable button
          scanReceiptBtn.disabled = false;

          // Update inline status box with success confirmation
          if (receiptScanStatusBox) {
            // Display status box
            receiptScanStatusBox.style.display = 'block';
            // Set success background styling
            receiptScanStatusBox.style.background = 'rgba(16, 185, 129, 0.12)';
            // Set success text color
            receiptScanStatusBox.style.color = '#34d399';
            // Set success border styling
            receiptScanStatusBox.style.border = '1px solid rgba(16, 185, 129, 0.3)';
            // Format success status text
            const merchantName = result.merchant ? `<strong>${result.merchant}</strong>` : 'receipt';
            // Format amount display string
            const amountStr = result.amount ? ` for <strong>${result.amount}</strong>` : '';
            // Set HTML content of status box
            receiptScanStatusBox.innerHTML = `✅ Successfully extracted ${merchantName}${amountStr}. Verify details below and click Save!`;
          // End success status box update
          }

          // Reset button back to default label after 3.5 seconds
          setTimeout(() => {
            // Restore default button label
            scanReceiptBtn.innerHTML = '📷 Snap or Upload Receipt Photo';
            // Restore default button color
            scanReceiptBtn.style.color = '#a855f7';
            // Restore default button border
            scanReceiptBtn.style.borderColor = 'rgba(168, 85, 247, 0.3)';
          // End timeout delay
          }, 3500);

        // Catch and handle any errors during receipt scanning or API execution
        } catch (ocrErr) {
          // Log detailed error to developer console
          console.error('Receipt OCR Scanning Error:', ocrErr);
          // Update button to indicate failure
          scanReceiptBtn.innerHTML = '❌ Scan Failed';
          // Set error color on button
          scanReceiptBtn.style.color = '#ef4444';
          // Re-enable button
          scanReceiptBtn.disabled = false;

          // Update inline status box with failure notification
          if (receiptScanStatusBox) {
            // Display status box
            receiptScanStatusBox.style.display = 'block';
            // Set error background on status box
            receiptScanStatusBox.style.background = 'rgba(239, 68, 68, 0.12)';
            // Set error text color on status box
            receiptScanStatusBox.style.color = '#f87171';
            // Set error border on status box
            receiptScanStatusBox.style.border = '1px solid rgba(239, 68, 68, 0.3)';
            // Set error message text in status box
            receiptScanStatusBox.innerHTML = `<strong>❌ Scan Error:</strong> ${ocrErr.message || 'Could not analyze receipt photo.'} Please enter details manually or check your Gemini API key.`;
          // End error status box update
          }

          // Display modal alert informing user of error
          alert(`❌ Receipt Scan Error\n\n${ocrErr.message || 'Could not analyze receipt image.'}\n\nPlease check your internet connection or Gemini API key in Settings, or enter details manually.`);
        ...
        // Clean up file input value to allow selecting the same file again
        } finally {
